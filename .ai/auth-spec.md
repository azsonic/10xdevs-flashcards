...
- **Schema Validation**: Use `zod` for all form inputs.
  - _Email_: Valid email format.
  - _Password_: Min 6 characters (Supabase default).
- **Error UI**:
...
- **Scenarios**:
  - _Invalid Credentials_: Show "Invalid email or password".
  - _Existing User_: On registration, show "User already registered".
  - _Weak Password_: Show specific requirements.
